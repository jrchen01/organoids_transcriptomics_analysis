...
seurat <- subset(seurat, subset = cell.major != "Unknown")

run_cellchat <- function(seurat_group, group_by) {
  matrix_data <- seurat_group@assays$RNA$data %>% na.omit()
  cellchat <- createCellChat(object = matrix_data, meta = seurat_group@meta.data, group.by = group_by)
...
saveRDS(cellchat_sub_ad, "/home/jchen/data1/jchen/project/01_sarah_proj/05_organoids/02_results/01_ccc/ccc_11mo_non_protein_AD_subtype.rds")
saveRDS(cellchat_sub_nc, "/home/jchen/data1/jchen/project/01_sarah_proj/05_organoids/02_results/01_ccc/ccc_11mo_non_protein_NC_subtype.rds")

## major cell-class level - not in Fig 5, kept for reference
cellchat_major_ad <- run_cellchat(subset(seurat, subset = diagnosis == "sAD"), group_by = "cell.major")
cellchat_major_nc <- run_cellchat(subset(seurat, subset = diagnosis == "CTRL"), group_by = "cell.major")

saveRDS(cellchat_major_ad, "/home/jchen/data1/jchen/project/01_sarah_proj/05_organoids/02_results/01_ccc/ccc_11mo_non_protein_AD_major.rds")
saveRDS(cellchat_major_nc, "/home/jchen/data1/jchen/project/01_sarah_proj/05_organoids/02_results/01_ccc/ccc_11mo_non_protein_NC_major.rds")

# Fig 5c - circle plots, Fig 5d - outgoing/incoming signal bar graphs
...
pdf("/home/jchen/data1/jchen/project/01_sarah_proj/05_organoids/02_results/01_ccc/plots/11mo_ccc_non_protein_signals_subtype.pdf", width = 12, height = 10)
...
pdf("/home/jchen/data1/jchen/project/01_sarah_proj/05_organoids/02_results/01_ccc/plots/11mo/11mo_ccc_non_protein_signals_subtype_barplot_final.pdf", width = 10, height = 5)
print(combined_ad_send)
print(combined_ad_receive)
dev.off()

# differential chord diagram (AD - NC), normalized to equal total signal
...
pdf("/home/jchen/data1/jchen/project/01_sarah_proj/05_organoids/02_results/01_ccc/plots/11mo/11mo_INH4_outgoing_differential.pdf", width = 8, height = 8)
my_netVisual_chord(
  diff_matrix, sources.use = "Inhibitory 4", targets.use = NULL,
  edge.transparency = TRUE, show.legend = TRUE, legend.pos.y = 20,
  title.name = "Inhibitory 4 Differential Outgoing Signals\n(AD - NC)",
  vertex.label.cex = 0.8, small.gap = 1, big.gap = 10,
  remove.isolate = FALSE, directional = 1, title.cex = 1.2  # range_value defaults to max(abs(net$prob))
)
dev.off()
