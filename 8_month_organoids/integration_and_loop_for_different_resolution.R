...
## check cell count per patient per cluster
output_dir <- "/raidixshare_logg01/jchen/project/01_sarah_proj/01_jupyternotebook_files/plots/month8_seurat/diff_resol/count/"
resolutions <- seq(0.05, 0.3, by = 0.05)
seurat.8mo <- FindNeighbors(seurat.8mo, reduction = "integrated.cca", dims = 1:20) # dim = 20

for (res in resolutions) { # dim = 20
  seurat.8mo <- FindClusters(seurat.8mo, resolution = res)
  seurat.8mo <- RunUMAP(seurat.8mo, dims = 1:20, reduction = "integrated.cca")
  cluster_patient_counts <- seurat.8mo@meta.data %>%
    dplyr::count(seurat_clusters, patient_id) %>%
    dplyr::rename(Cluster = seurat_clusters, PatientID = patient_id, CellCount = n)
  hist_plot <- ggplot(cluster_patient_counts, aes(x = Cluster, y = CellCount, fill = PatientID)) +
    geom_bar(stat = "identity", position = "dodge") +
    labs(title = paste("cell count per patient per clusters | dim20 | resolution:", res),
         x = "Cluster", y = "Cell Count") +
    theme_minimal() +
    theme(panel.background = element_rect(fill = "white", color = NA),
        plot.background = element_rect(fill = "white", color = NA))
  ggsave(filename = paste0(output_dir, "count_dim20_resol_", res, ".png"), plot = hist_plot, width = 8, height = 6)
}

seurat.8mo <- FindNeighbors(seurat.8mo, reduction = "integrated.cca", dims = 1:30) # dim = 30
for (res in resolutions) { # dim = 30
  seurat.8mo <- FindClusters(seurat.8mo, resolution = res)
  seurat.8mo <- RunUMAP(seurat.8mo, dims = 1:30, reduction = "integrated.cca")
  cluster_patient_counts <- seurat.8mo@meta.data %>%
    dplyr::count(seurat_clusters, patient_id) %>%
  ...
  hist_plot <- ggplot(cluster_patient_counts, aes(x = Cluster, y = CellCount, fill = PatientID)) +
    geom_bar(stat = "identity", position = "dodge") +
    labs(title = paste("cell count per patient per clusters | dim30 | resolution:", res),
         x = "Cluster", y = "Cell Count") +
    theme_minimal() +
    theme(panel.background = element_rect(fill = "white", color = NA),
        plot.background = element_rect(fill = "white", color = NA))
  ggsave(filename = paste0(output_dir, "count_dim30_resol_", res, ".png"), plot = hist_plot, width = 8, height = 6)
}
