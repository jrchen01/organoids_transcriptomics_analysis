...
pdf("/raidixshare_logg01/jchen/project/01_sarah_proj/05_organoids/02_results/05_clustering/clustering_umap/8mo_umap_phase.pdf", width = 8, height = 8)
p_phase_8mo
dev.off()
...
pdf("/raidixshare_logg01/jchen/project/01_sarah_proj/05_organoids/02_results/06_SOX9/8mo_gobp_featureplot.pdf", width = 8, height = 8)
print(p_gobp_mo8)
dev.off()

## Fig 3A - SOX9 expression vs translation module score correlation
a <- FetchData(seurat, vars = c("GOBP_Translation1", "SOX9", "HIF1A"))
cor_result <- cor.test(a$GOBP_Translation1, a$SOX9)
print(cor_result)

p_sox9_cor_mo8 <- ggplot(a, aes(SOX9, GOBP_Translation1)) +
  geom_point() +
...
pdf("/raidixshare_logg01/jchen/project/01_sarah_proj/05_organoids/02_results/06_SOX9/8mo_sox9_corr.pdf", width = 8, height = 8)
p_sox9_cor_mo8
dev.off()
